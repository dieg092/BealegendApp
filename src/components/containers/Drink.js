...
class Drink extends Component {
  ...
  onProductPressed(product) {
    if (this.props.user.rol === CONSTANTS.ROLES.BOSS ||
      ...
      this.props.cleanCode();
      this.toggleModalConsumeDrink();
    }
  }

  onConsumeAceptedPress() {
    this.props.consumeAcepted(this.props.code, this.props.user, this.state.productSelected.ticketsConsume, 'BEBIDA');
  }
  ...
  toggleModalConsumeDrink() {
    this.props.isModalOpen('BEBIDA');
  }
  ...
  renderModal() {
      return (
        <Modal
          ...
          backdropOpacity={0.5}
          isVisible={this.props.isModalOpenedDrink}
          onRequestClose={() => { this.toggleModalConsumeDrink(); }}
        >
          <View style={styles.modalContainer}>
            <Text style={styles.innerContainer}>CÓDIGO:</Text>
              ...
              <ButtonError onPress={() => this.toggleModalConsumeDrink()}>CANCELAR</ButtonError>
              <ButtonSuccess
  ...
  render() {
    return (
      ...
      >
        {this.props.bebidas &&
          this.props.bebidas.map((product, key) => {
            return (
              <View key={key}>
              <TouchableHighlight
...
const mapStateToProps = ({ product, auth }) => {
  const { products, bebidas, code, errorCode, isModalOpenedDrink } = product;

  return { products, bebidas, code, errorCode, isModalOpenedDrink };
};
