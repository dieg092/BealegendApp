...
class Food extends Component {
  ...
  onProductPressed(product) {
    if (this.props.user.rol === CONSTANTS.ROLES.BOSS ||
      ...
      this.props.cleanCode();
      this.toggleModalConsumeFood();
    }
  }

  onConsumeAceptedPress() {
    this.props.consumeAcepted(this.props.code, this.props.user, this.state.productSelected.ticketsConsume, 'COMIDA');
  }
  ...
  toggleModalConsumeFood() {
    this.props.isModalOpen('COMIDA');
  }
  ...
  renderModal() {
      return (
        <Modal
          ...
          backdropOpacity={0.5}
          isVisible={this.props.isModalOpenedFood}
          onRequestClose={() => { this.toggleModalConsumeFood(); }}
        >
          <View style={styles.modalContainer}>
            <Text style={styles.innerContainer}>CÓDIGO:</Text>
              ...
              <ButtonError onPress={() => this.toggleModalConsumeFood()}>CANCELAR</ButtonError>
              <ButtonSuccess
  ...
  render() {
    return (
      ...
      >
        {this.props.comidas &&
          this.props.comidas.map((product, key) => {
            return (
              <View key={key}>
              <TouchableHighlight
...
const mapStateToProps = ({ product }) => {
  const { products, comidas, code, errorCode, isModalOpenedFood } = product;

  return { products, comidas, code, errorCode, isModalOpenedFood };
};
