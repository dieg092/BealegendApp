...
class Computer extends Component {
  ...
  componentWillMount() {
    this.props.getProducts('ORDENADOR');
  ...
  onProductPressed(product) {
    if (this.props.user.rol === CONSTANTS.ROLES.BOSS ||
      ...
      this.props.cleanCode();
      this.toggleModalConsumeComputer();
    }
  }

  onConsumeAceptedPress() {
    this.props.consumeAcepted(this.props.code, this.props.user, this.state.productSelected.ticketsConsume, 'ORDENADOR');
  }
  ...
  toggleModalConsumeComputer() {
    this.props.isModalOpen('ORDENADOR');
  }
  ...
  renderModal() {
      return (
        <Modal
          ...
          backdropOpacity={0.5}
          isVisible={this.props.isModalOpenComputer}
          onRequestClose={() => { this.toggleModalConsumeComputer(); }}
        >
          <View style={styles.modalContainer}>
            <Text style={styles.innerContainer}>CÓDIGO:</Text>
              ...
              <ButtonError onPress={() => this.toggleModalConsumeComputer()}>CANCELAR</ButtonError>
              <ButtonSuccess
  ...
  render() {
    return (
      ...
      >
        {this.props.ordenadores &&
          this.props.ordenadores.map((product, key) => {
            return (
              <View key={key}>
              <TouchableHighlight
...
const mapStateToProps = ({ product }) => {
  const { products, ordenadores, code, errorCode, isModalOpenComputer } = product;

  return { products, ordenadores, code, errorCode, isModalOpenComputer };
};
